...
<xsl:key name="sampleunitlookupid"  match="//SAMPLE_SET/SAMPLE/SAMPLE_ATTRIBUTES/SAMPLE_ATTRIBUTE/UNITS"  use="."/>
 ...
 <xsl:param name="experimentDoc" select="document('http://www.ebi.ac.uk/ena/data/view/SRX044491-SRX044508&amp;display=xml')"/>
...
-->
 ...
 <xsl:variable name="submission" select="document('http://www.ebi.ac.uk/ena/data/view/ERA000092&amp;display=xml')"/>
...
<!--
<xsl:if test="child::IDENTIFIERS/SUBMITTER_ID">
 <xsl:text>DUH!</xsl:text><xsl:text>&#9;</xsl:text>
 <xsl:value-of select="child::IDENTIFIERS/SUBMITTER_ID/."></xsl:value-of>
<xsl:text>
</xsl:text>
 </xsl:if> 
...
<xsl:if test="child::TITLE">
 ...
 <xsl:value-of select="child::TITLE/."/><xsl:text>&#9;</xsl:text>
 <xsl:text>
</xsl:text>
</xsl:if> 
-->
...
<xsl:text>
    ...
    <xsl:text>>>> applying SAMPLE Template to:  </xsl:text> <xsl:value-of select="./ID/."/>      <xsl:text>&#9;</xsl:text>
      <xsl:text>   
</xsl:text>
      <xsl:variable name="samples" select="./ID/."/>
     <!-- <xsl:value-of select="document(concat('http://www.ebi.ac.uk/ena/data/view/',$samples,'&amp;display=xml'))"/><xsl:text>&#9;</xsl:text> -->
      <xsl:text>   
      </xsl:text>
      <xsl:text>
      </xsl:text>
      ...
      <xsl:text>Characteristics[primary accession number]</xsl:text><xsl:text>&#9;</xsl:text>
      <xsl:text>Comment[Scientific Name]</xsl:text><xsl:text>&#9;</xsl:text>
      <xsl:text>Characteristics[Taxonomic ID]</xsl:text><xsl:text>&#9;</xsl:text>
...
<xsl:text>   
     ...
     <!-- <xsl:variable name="experimentDoc" select="document(concat('http://www.ebi.ac.uk/ena/data/view/',$experiments,'&amp;display=xml'))"/> -->
    
     <!-- <xsl:value-of select="document(concat('http://www.ebi.ac.uk/ena/data/view/',$experiments,'&amp;display=xml'))"/><xsl:text>&#9;</xsl:text> -->
     ...
     <xsl:when test="contains(./DB/., 'NA-FASTQ-FILES')">
     <!-- <xsl:value-of select="./ID/."/> -->
      <xsl:variable name="fastqs" select="./ID/."/>
